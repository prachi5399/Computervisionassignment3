...
def img_alignment(img1, img2):
    ...
    if warp_mode == cv2.MOTION_HOMOGRAPHY:
        img2_aligned = cv2.warpPerspective(img2, warp_matrix, (image_size[1], image_size[0]), flags= cv2.INTER_LINEAR + cv2.WARP_INVERSE_MAP)
    else:
        img2_aligned = cv2.warpAffine(img2, warp_matrix, (image_size[1], image_size[0]), flags= cv2.INTER_LINEAR + cv2.WARP_INVERSE_MAP)
